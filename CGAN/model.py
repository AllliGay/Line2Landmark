...
class pix2pix(object):
    ...
    def test(self, args):
        ...
        for i, sample_image in enumerate(sample_images):
            ...
            image_name = sample_files[i].split('/')[-1]
            save_images(samples, [self.batch_size, 1],
                        './{}/{}'.format(args.test_dir, image_name))

    def predict(self, img):
        self.load(self.checkpoint_dir, True)
        if type(img) is str:
            img = load_data(img, is_test=True)
            img = img[self.output_c_dim:]
        img = img.reshape((1, self.image_size, self.image_size, 1))
        img = np.concatenate((np.zeros((1,self.image_size, self.image_size,3)),img),axis=-1)

        result = self.sess.run(
            self.fake_B_sample,
            feed_dict={self.real_data: img}
        )
        result = convert_result_images(result, [self.batch_size, 1])
        return result
