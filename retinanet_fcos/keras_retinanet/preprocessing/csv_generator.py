...
def _read_annotations(csv_reader, classes, base_dir):
    ...
    for line, row in enumerate(csv_reader):
        ...
        try:
            img_file, points = row
            y1,x1,y2,x2 = points.split()
        except ValueError:
            raise_from(ValueError('line {}: format should be \'img_file,x1,y1,x2,y2,class_name\' or \'img_file,,,,,\''.format(line)), None)
        
        if os.path.exists(base_dir+'/{}.jpg'.format(img_file))==0:
            continue
        if max(float(x2)-float(x1),float(y2)-float(y1)) * 600 < 400:
            continue
...
class CSVGenerator(Generator):
    ...
    def __init__(
        self,
        csv_data_file='/home/zjc793532302/GoogleLandmark/boxes.csv',
        csv_class_file=None,
        base_dir=None,
        **kwargs
    ):
        ...
        if self.base_dir is None:
            self.base_dir = os.path.dirname(csv_data_file)+'/train/train_data'
